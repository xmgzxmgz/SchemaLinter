...
class PythonParser(BaseParser):
    ...
    def parse_file(self, file_path: str) -> List[CodeReference]:
        ...
        try:
            ...
            print(f"警告: 解析Python文件 {file_path} 时出错: {e}")
    ...
    def _parse_sql_strings(self, tree: ast.AST, file_path: str, content: str) -> List[CodeReference]:
        ...
        class SQLStringVisitor(ast.NodeVisitor):
            ...
            def _extract_sql_references(self, sql_content: str) -> tuple:
                ...
                # 提取表名
                table_matches = re.findall(self.parser.table_pattern, sql_content, re.IGNORECASE)
                tables.update(table_matches)
                
                # 简化的列名提取（实际应用中需要更复杂的解析）
                # 这里只是示例，实际需要更精确的SQL解析
                column_matches = re.findall(r'\b([a-zA-Z_][a-zA-Z0-9_]*)\s*[=<>!]', sql_content)
                
                # 假设列属于第一个找到的表（简化处理）
                if tables and column_matches:
                    first_table = list(tables)[0]
                    for column in column_matches:
                        if column.upper() not in ['SELECT', 'FROM', 'WHERE', 'AND', 'OR']:
                            columns.append((first_table, column))
